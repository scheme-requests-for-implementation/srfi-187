	  ...
	  (let ((rest (cdr pattern)))
	    (cond
	     ...
	     ((symbol? rest)		;(& . rest)
	      (if (null? rem)
		  (if (every-single? req)
		      `(lambda (,@(map car req) & . ,rest) ,@bd)
		      (let* ((vs (map car req))
			     (gen-vs (pair->gensym-pair vs))
			     (gr (gensym))
			     (gv (gensym)))
			`(lambda (,@gen-vs ,gr . ,gv)
			   (let* (,@(map (lambda (v g t) `(,v (wow-opt ,v ,g ,@t))) vs gen-vs (map cdr req)) (& ,gr) (,rest ,gv))
			     ,@bd))))
		  (let ((z (car rem)))
		    `(let* ((& (if (null? ,z) ,(car df) (car ,z)))
			    (,rest (if (null? ,z) ,z (cdr ,z))))
		       ,@bd))))
	     (else			;(& a a1 ...)
	      (if (null? rem)
		  (let ((z (gensym)))
		  ...
		  (let ((n (cadr var)))
			...
			(let ((z (car rem)))
			  (if (null? df)
				 ...
				 ,(lamb df req rem (cdr pattern) bd)))))))
		 ((and (pair? v) (eq? 'quote (car v))) ;(('n k f ...) d t ...)
		  (let ((n (cadr v)))
		    (if (null? rem)
			(let ((z (gensym))
			      (vs (map car req)))
